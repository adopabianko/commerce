...
func (uc *PlaceOrder) Exec(ctx context.Context, req Request) (*Response, error) {
	...
	for _, it := range req.Items { o.Items = append(o.Items, order.OrderItem{SKU: it.SKU, Qty: it.Qty}) }
	if err := uc.repo.CreateOrder(o); err != nil { return nil, err }

	ctxTO, cancel := context.WithTimeout(ctx, uc.timeout); defer cancel()

	chk := &inventoryv1.CheckStockRequest{}
	for _, it := range req.Items { chk.Items = append(chk.Items, &inventoryv1.Item{Sku: it.SKU, Qty: it.Qty}) }
	resChk, err := uc.inv.CheckStock(ctxTO, chk)
	if err != nil || !resChk.Ok { _ = uc.repo.UpdateStatus(id, "REJECTED"); return &Response{OrderID: id, Status: "REJECTED"}, nil }

	resReq := &inventoryv1.ReserveStockRequest{}
	for _, it := range req.Items { resReq.Items = append(resReq.Items, &inventoryv1.Item{Sku: it.SKU, Qty: it.Qty}) }
	resRes, err := uc.inv.ReserveStock(ctxTO, resReq)
	if err != nil || !resRes.Ok { _ = uc.repo.UpdateStatus(id, "REJECTED"); return &Response{OrderID: id, Status: "REJECTED"}, nil }
