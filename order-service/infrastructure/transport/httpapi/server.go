...
func (h *Handler) placeOrder(c *gin.Context) {
	var req struct{ Items []struct{ SKU string `json:"sku"`; Qty int32 `json:"qty"` } `json:"items"` }
	...
	for _, it := range req.Items { items = append(items, usecase.Item{SKU: it.SKU, Qty: it.Qty}) }
